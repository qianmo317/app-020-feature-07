...
4. **人数与自动校验**（编辑后 500ms 防抖重算，不需要点按钮）：每个房间可填实际人数（留空按用途密度估算），房间名下方在图上直接显示实填/估算人数；自动校核疏散最远距离、袋形走道死端、灭火器未覆盖面积、安全出口数量与连通性、**各出口按最近出口分流的人数与净宽度容量（超载出口图上标红转圈并注明超员数）、楼层所需/现有疏散总净宽度**、房间无门、检查记录过期/缺失/损坏。
5. **规则可配**：`#/rules` 按建筑类别改限值（疏散距离、袋形走道、灭火器半径、需两个出口的最小面积与人数、单出口最小净宽度、每百人疏散净宽度、依据文号），改一次版本号 +1，此后校验结果记录当时快照。
...
6. **安全出口数量**：`required = (楼层总面积 > exitMinAreaM2 或 人数 > exitMaxOccupants) ? 2 : 1`；人数取房间实填 `occupants`，留空时按用途密度估算（办公 10、商业 3、仓库 50、病房 8、走道 0、其他 20 ㎡/人）。
7. **疏散宽度与出口分流**（`engine.ts`）：百人宽度指标按楼层取 GB 50016 表 5.5.21-1（地上 1-2 层 0.65、3 层 0.75、≥4 层 1.00 m/百人，地下保守 1.00），且不窄于规则页配置值；所需总净宽度 = `人数 × 指标 / 100`。每个房间按「房心→门直线 + 门→出口走道路径」分配给最近出口（开敞大空间按直线近似；走道仅在显式填人数时参与）；单口容量 = `净宽 ÷ 指标 × 100`（默认净宽 0.9m，GB 50016 5.5.19，可在设施属性里改），分配人数超容量即 `EXIT_CAPACITY` 并在图上红色虚线转圈告警、标注「分配/容量 人（超 N）」；总净宽不足出 `EXIT_WIDTH`（已有单口超载时降为 warning 汇总），单口窄于最小值出 `EXIT_WIDTH_NARROW`。
...
10. **渲染**：全部走 SVG，1 用户单位 = 1mm，缩放是 viewBox 变换（不重算几何）；图纸含 1m/5m 网格、房间多边形与面积/人数标注（实填加粗、估算灰色「约 N 人」）、设施符号、超载出口告警、未覆盖栅格高亮、校验定位红圈。
11. **状态管理**（`src/store/store.ts`）：手写外部 store + `useSyncExternalStore`；每次 `setState` 浅拷贝各顶层容器并替换被改动的对象引用，保证选择器能感知更新（`tests/store.test.ts` S1 就是这条的回归）；旧版 localStorage 数据加载时按字段合并 `DEFAULT_RULES`，补齐新增规则字段。
